...
function EditStudent() {
    ...
    const editBatch=async(e)=>{
      ...
      cource
        ...
        console.log(body);
        const result = await axios.post('http://localhost:8000/updateStudent',body,{
          headers: {
    ...
    const fetchEmp = async () => {
        const {data} = await axios.get('http://localhost:8000/editStudent/'+params.id)
        setUname(data.student.name)
        setAge(data.student.age)
        setGender(data.student.gender)
        setAddress(data.student.address)
        setDob(data.student.dob)
        setPlace(data.student.place)
        setPin(data.student.pin)
        setPhoneno(data.student.phonenumber)
        setBatchId(data.student.batchId)
        setEmail(data.student.email)
        setCource(data.student.cource)
    }
      ...
      <option value="Python Django">PYTHON DJANGO</option>
                    ...
                    <Form.Group controlId="formGridEmail" className='mt-3'>
                        <div className='row'>
                            ...
                            <span className='col-1 text-end mt-1'> : </span>
                            <div className='col-7'><Form.Control type="text" value={batchId} onChange={(e) => setBatchId(e.target.value)} placeholder="Batch ID" /></div>
                        </div>
                    </Form.Group>
                    ...
                    {/* <Form.Group controlId="formGridEmail" className='mt-3'>
                        <div className='row'>
                            ...
                            <span className='col-1 text-end mt-1'> : </span>
                            <div className='col-7'><Form.Control value={tutorName} onChange={(e) => setTutorName(e.target.value.toUpperCase())} type="text" placeholder="Tutor Name" /></div>
                        </div>
                    </Form.Group> */}
      ...
      <option id='OTHER'  value="OTHER">Other</option>
          ...
          <Form.Group controlId="formGridEmail" className='mt-3'>
            <div className='row'>
              ...
              <span className='col-1 text-end mt-1'> : </span>
              <div className='col-7'><Form.Control value={pin} onChange={(e) => setPin(e.target.value.toUpperCase())} type="number" placeholder="pin code" /></div>
            </div>
          </Form.Group>
